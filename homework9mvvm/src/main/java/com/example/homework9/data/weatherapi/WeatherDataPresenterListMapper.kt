...
class WeatherDataPresenterListMapper(private val tempUnitType: TempUnitType) :
        (List<WeatherData>) -> List<WeatherDataPresenter> {
    ...
    private fun mapWithFahrenheit(weatherDataList: List<WeatherData>): MutableList<WeatherDataPresenter> {
        ...
        for (i in weatherDataList.indices) {
            weatherDataPresentModelList.add(
                WeatherDataPresenter(
                    cityName = weatherDataList[i].cityName,
                    date = weatherDataList[i].date,
                    iconType = weatherDataList[i].iconType,
                    temperature = DecimalFormat("###.#").format(kelvinToFahrenheit(weatherDataList[i].temperatureKelvin))
                        .toDouble(),
        ...
        for (i in weatherDataList.indices) {
            weatherDataPresentModelList.add(
                WeatherDataPresenter(
                    ...
                    date = weatherDataList[i].date,
                    iconType = weatherDataList[i].iconType,
                    temperature = DecimalFormat("###.#").format(kelvinToCelsius(weatherDataList[i].temperatureKelvin))
                        .toDouble(),
                    weather = weatherDataList[i].weather
                )
            )
